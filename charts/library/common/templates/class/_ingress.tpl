...
{{- define "asa.v1.common.class.ingress" -}}

  {{- $rootCtx := .rootCtx -}}
  {{- $objectData := .objectData -}}

  {{- $svcData := (include "asa.v1.common.lib.ingress.targetSelector" (dict "rootCtx" $rootCtx "objectData" $objectData) | fromYaml) -}}
  ...
  */}}
  {{- if (include "asa.v1.common.lib.util.stopAll" $rootCtx) -}}
    {{- $ingressClassName = "asa-stopped" -}}
  {{- end -}}

  {{- include "asa.v1.common.lib.ingress.integration.nginx" (dict "rootCtx" $rootCtx "objectData" $objectData) -}}
  {{- if ne $ingressClassName "asa-stopped" -}}{{/* If is stopped, dont render homepage annotations */}}
    {{- include "asa.v1.common.lib.ingress.integration.homepage" (dict "rootCtx" $rootCtx "objectData" $objectData) -}}
  {{- end }}
---
...
metadata:
  ...
  namespace: {{ include "asa.v1.common.lib.metadata.namespace" (dict "rootCtx" $rootCtx "objectData" $objectData "caller" "Ingress") }}
  ...
  rules:
    ...
    - host: {{ $h.host | quote }}
      http:
        paths:
          {{- range $p := $h.paths -}}
            {{- $newSvcData := (include "asa.v1.common.lib.ingress.backend.data" (dict
