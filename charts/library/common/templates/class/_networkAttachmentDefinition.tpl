...
{{/* Call this template:
{{ include "asa.v1.common.class.networkAttachmentDefinition" (dict "rootCtx" $ "objectData" $objectData) }}
...
{{- define "asa.v1.common.class.networkAttachmentDefinition" -}}
...
metadata:
  ...
  namespace: {{ include "asa.v1.common.lib.metadata.namespace" (dict "rootCtx" $rootCtx "objectData" $objectData "caller" "Network Attachment Definition") }}
  {{- $labels := (include "asa.v1.common.lib.metadata.allLabels" $rootCtx | fromYaml) | default dict -}}
  {{- with (include "asa.v1.common.lib.metadata.render" (dict "rootCtx" $rootCtx "labels" $labels) | trim) }}
  labels:
    {{- . | nindent 4 }}
  {{- end -}}
  {{- $annotations := (include "asa.v1.common.lib.metadata.allAnnotations" $rootCtx | fromYaml) | default dict -}}
