...
{{- define "asa.v1.common.class.service" -}}
  ...
  {{- if not (mustHas $svcType $specialTypes) -}}
    ...
    {{- $podValues = fromJson (include "asa.v1.common.lib.helpers.getSelectedPodValues" (dict "rootCtx" $rootCtx "objectData" $objectData "caller" "Service")) -}}

    {{- if $podValues -}}
      {{/* Get Pod hostNetwork configuration */}}
      {{- $hostNetwork = include "asa.v1.common.lib.pod.hostNetwork" (dict "rootCtx" $rootCtx "objectData" $podValues) -}}
  ...
  {{- if eq $objectData.type "LoadBalancer" -}}
    {{- include "asa.v1.common.lib.service.loadbalancer.validate" (dict "objectData" $objectData) -}}
    {{- include "asa.v1.common.lib.service.integration.cilium" (dict "rootCtx" $rootCtx "objectData" $objectData) -}}
  {{- end -}}
---
apiVersion: v1
kind: Service
metadata:
  ...
  namespace: {{ include "asa.v1.common.lib.metadata.namespace" (dict "rootCtx" $rootCtx "objectData" $objectData "caller" "Service") }}
  {{- $labels := (mustMerge ($objectData.labels | default dict) (include "asa.v1.common.lib.metadata.allLabels" $rootCtx | fromYaml)
                            (include "asa.v1.common.lib.metadata.selectorLabels" (dict "rootCtx" $rootCtx "objectType" "service" "objectName" $objectData.shortName) | fromYaml)) -}}
...
spec:
  {{- if eq $objectData.type "ClusterIP" -}}
    {{- include "asa.v1.common.lib.service.spec.clusterIP" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim | nindent 2 -}}
  {{- else if eq $objectData.type "LoadBalancer" -}}
    {{- include "asa.v1.common.lib.service.spec.loadBalancer" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim | nindent 2 -}}
  {{- else if eq $objectData.type "NodePort" -}}
    {{- include "asa.v1.common.lib.service.spec.nodePort" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim | nindent 2 -}}
  {{- else if eq $objectData.type "ExternalName" -}}
    {{- include "asa.v1.common.lib.service.spec.externalName" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim | nindent 2 -}}
  {{- else if eq $objectData.type "ExternalIP" -}}
    {{- include "asa.v1.common.lib.service.spec.externalIP" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim | nindent 2 -}}
  {{- end -}}
  {{- with (include "asa.v1.common.lib.service.ports" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim) }}
  ports:
    {{- . | nindent 4 }}
  {{- end -}}
  ...
  selector:
    ...
    {{- else }}
      {{- include "asa.v1.common.lib.metadata.selectorLabels" (dict "rootCtx" $rootCtx "objectType" "pod" "objectName" $podValues.shortName) | trim | nindent 4 -}}
    {{- end }}
  {{- end -}}
  ...
  {{- if eq $objectData.type "ExternalIP" -}}
    ...
    {{- end -}}
    {{- if $useSlice -}}
      {{- include "asa.v1.common.class.endpointSlice" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim | nindent 0 }}
    {{- else -}}
      {{- include "asa.v1.common.class.endpoint" (dict "rootCtx" $rootCtx "objectData" $objectData) | trim | nindent 0 }}
    {{- end -}}
  {{- end -}}
{{- end -}}
