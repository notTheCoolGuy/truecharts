{{- define "asa.v1.common.class.podmonitor" -}}
...
metadata:
  ...
  {{- $labels := (mustMerge ($podmonitorLabels | default dict) (include "asa.v1.common.lib.metadata.allLabels" $ | fromYaml)) -}}
  {{- with (include "asa.v1.common.lib.metadata.render" (dict "rootCtx" $ "labels" $labels) | trim) }}
  labels:
    {{- . | nindent 4 }}
  {{- end }}
  {{- $annotations := (mustMerge ($podmonitorAnnotations | default dict) (include "asa.v1.common.lib.metadata.allAnnotations" $ | fromYaml)) -}}
...
spec:
  ...
  selector:
    ...
    matchLabels:
      {{- include "asa.v1.common.lib.metadata.selectorLabels" (dict "rootCtx" $ "objectType" "pod" "objectName" $selectedPodName) | indent 6 }}
    {{- end }}
  podMetricsEndpoints:
