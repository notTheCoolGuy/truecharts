{{- define "asa.v1.common.class.prometheusrule" -}}
...
metadata:
  ...
  {{- $labels := (mustMerge ($prometheusruleLabels | default dict) (include "asa.v1.common.lib.metadata.allLabels" $ | fromYaml)) -}}
  {{- with (include "asa.v1.common.lib.metadata.render" (dict "rootCtx" $ "labels" $labels) | trim) }}
  labels:
    {{- . | nindent 4 }}
  {{- end }}
  {{- $annotations := (mustMerge ($prometheusruleAnnotations | default dict) (include "asa.v1.common.lib.metadata.allAnnotations" $ | fromYaml)) -}}
