{{- define "asa.v1.common.class.servicemonitor" -}}
...
metadata:
  ...
  {{- $labels := (mustMerge ($servicemonitorLabels | default dict) (include "asa.v1.common.lib.metadata.allLabels" $ | fromYaml)) -}}
  {{- with (include "asa.v1.common.lib.metadata.render" (dict "rootCtx" $ "labels" $labels) | trim) }}
  labels:
    {{- . | nindent 4 }}
  {{- end }}
  {{- $annotations := (mustMerge ($servicemonitorAnnotations | default dict) (include "asa.v1.common.lib.metadata.allAnnotations" $ | fromYaml)) -}}
...
spec:
  ...
  selector:
    ...
    {{- $selectedService := fromYaml ( include "asa.v1.common.lib.helpers.getSelectedServiceValues" (dict "rootCtx" $ "objectData" $objectData)) }}
    {{- $selectedServiceName := $selectedService.shortName }}
    matchLabels:
      {{- include "asa.v1.common.lib.metadata.selectorLabels" (dict "rootCtx" $ "objectType" "service" "objectName" $selectedServiceName) | indent 6 }}
    {{- end }}
  endpoints:
