{{/* PVC Spawner */}}
{{/* Call this template:
{{ include "asa.v1.common.spawner.pvc" $ -}}
*/}}

{{- define "asa.v1.common.spawner.pvc" -}}
  ...
  {{- range $name, $persistence := .Values.persistence -}}
    ...
    {{- if eq $enabled "true" -}}
      ...
      {{- include "asa.v1.common.lib.persistence.validation" (dict "rootCtx" $ "objectData" $objectData) -}}
      {{- include "asa.v1.common.lib.metadata.validation" (dict "objectData" $objectData "caller" "Persistence") -}}
      ...
      {{- if and (mustHas $objectData.type $types) (not $objectData.existingClaim) -}}
        ...
        {{- $_ := set $objectData "name" (include "asa.v1.common.lib.storage.pvc.name" (dict "rootCtx" $ "objectName" $name "objectData" $objectData)) -}}
        ...
        {{- if and $objectData.static $objectData.static.mode (ne $objectData.static.mode "disabled") -}}
          ...
          {{- if eq $objectData.static.mode "smb" -}}
            {{/* Validate SMB CSI */}}
            {{- include "asa.v1.common.lib.storage.smbCSI.validation" (dict "rootCtx" $ "objectData" $objectData) -}}
            ...
            {{- end -}}
            {{- include "asa.v1.common.class.secret" (dict "rootCtx" $ "objectData" $secretData) -}}
        ...
        {{- range $volSnap := $objectData.volumeSnapshots -}}
          ...
          {{- include "asa.v1.common.lib.chart.names.validation" (dict "name" $snapshotName "length" 253) -}}
          {{- include "asa.v1.common.lib.metadata.validation" (dict "objectData" $volSnapData "caller" "PVC - Volume Snapshot") -}}
          ...
          {{- include "asa.v1.common.lib.volumesnapshot.validation" (dict "objectData" $volSnapData) -}}

          {{/* Call class to create the object */}}
          {{- include "asa.v1.common.class.volumesnapshot" (dict "rootCtx" $ "objectData" $volSnapData) -}}
        {{- end -}}
      {{- end -}}
      ...
      {{- if eq $objectData.type "iscsi" -}}
        {{- if or $objectData.iscsi.authSession $objectData.iscsi.authDiscovery -}}
          ...
          {{- $secretData := (dict
                                ...
                                "data" (include "asa.v1.common.lib.storage.iscsi.chap" (dict "rootCtx" $ "objectData" $objectData) | fromJson)
                              ) -}}
          {{- include "asa.v1.common.class.secret" (dict "rootCtx" $ "objectData" $secretData) -}}
        {{- end -}}
      {{- end -}}
